...
pasS1 = elmNrs(y1:y2,zz:zz+5,:);
pasS_double = double(pasS1);
pasS = reshape(pasS_double, [], 1);

pasV1=elmNrs(y1:y2,zz+6:z2,:);
pasV_double = double(pasV1);
pasV2 = double(elmNrs([y1-1,y2+1],zz:z2,:));
pasV = union(pasV_double,pasV2);
...
x( act ) = volfrac0;
x( pasS ) = 0.1;                                                            % set x = 0.05 on pasS set
x( pasV ) = 0.0;
F0=zeros(nDof,1);
F = zeros(nDof,1);
...
line_load1 = 1050;  % 1050 kg/m  Tire load
line_load2 = 36000;  % 36000 kg/m  Concentrated load in the middle of the bridge
roll_length = round(0.6/ 14.6 * nely); % One tire width
...
live_load1_line = 10.0 * (1.4 * line_load1 * 91.25 * 3/ (1* (nelx + 1)* (floor_y + 1))) /  one_element_steel;
live_load2_line = 10.0 * (1.4 * line_load2 / ((nelx+1) * (floor_y + 1))) /  one_element_steel;

direction3 = 3;    
g4 = live_load1_line + dead_load + live_load2_line;   % dead_load + live_load
g3 = 10.0;
lcdof = 3*nodeNrs( y1:y2+1, zz+6, 1:nelx+1);

F0(lcdof) = -1.0;
F(:) = F0(:);
F(fixed)=0.0;
volfrac=(numel(act)*volfrac0+numel(pasS)*0.1+numel(pasV)*0.0)/nEl;
...
filename = 'xphys_four_bridge_data_xi.mat';
...
patch(isosurface(isovals, .2),'FaceColor',[192, 192, 192]/255,'EdgeColor','none'); %'b'
patch(isocaps(isovals, .2),'FaceColor','r','EdgeColor','none');

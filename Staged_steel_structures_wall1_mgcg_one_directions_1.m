...
act_wall1 = elmNrs(1:wall_width,1:nelz-7,1:85);
act_wall = reshape(act_wall1,[],1);

door_width = round(1000 / 9000 * 384);
door_height = round(2000 / 3000 * 128);

pasV_door1_x_1 = round(458.812 / 9000 * nelx + 1);
pasV_door1_x_2 = round(458.812 / 9000 * nelx + door_width);
pasV_door1_y_1 = 1;
pasV_door1_y_2 = wall_width;

pasV_door1 = elmNrs(pasV_door1_y_1:pasV_door1_y_2, 1: door_height, pasV_door1_x_1: pasV_door1_x_2);
pasV_door = reshape(pasV_door1, [], 1);

intersection = intersect(act_wall, pasV_door);
act_wall = setdiff(act_wall, intersection);

pasS_wall1_1 = elmNrs(1,1:nelz-7,1:85);
pasS_wall1_2 = elmNrs(13,1:nelz-7,1:85);
pasS_wall1 = union(pasS_wall1_1, pasS_wall1_2);
pasS_wall = double(pasS_wall1);

intersection = intersect(pasS_wall, pasV_door);
pasS_wall = setdiff(pasS_wall, intersection);

intersection = intersect(pasS_wall, act_wall);
act_wall = setdiff(act_wall, intersection);
% [y_indices, z_indices, x_indices] = ind2sub([nely, nelz, nelx],act_wall) ;
% 
...
% [y_indices, z_indices, x_indices] = ind2sub([nely, nelz, nelx],pasV) ;
% 
% % Display the 3D coordinates of each element in pasV2
% pasV2_index = [x_indices, y_indices, z_indices];
% 
% % Use the scatter3 function to draw the corresponding units
% scatter3(pasV2_index(:, 1), pasV2_index(:, 2), pasV2_index(:, 3), 'filled');
% xlabel('i');
% ylabel('j');
% zlabel('k');
% title('Visualization of pasV1\_index');
...
g2 = 3.0 * 10.0;
% direction1 = 1;
% g1 = 5.0 * 10.0 / sqrt(2.0);
pasS1 = double(reshape(pasS_wall1_1,[],1));
pasS2 = double(reshape(pasS_wall1_2,[],1));
...
filename = 'xphys_wall_1_data1.mat';
...
savefig(strcat('QNC_V1_staged_wall1_',num2str(loop),'.fig'));
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

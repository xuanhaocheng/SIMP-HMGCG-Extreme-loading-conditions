...
fixed1=3*nodeNrs( 1:nely+1, 1:nelz+1, 1)-2;
fixed2=3*nodeNrs( 1:nely+1, 1:nelz+1, 1)-1;
fixed3=3*nodeNrs( 1:nely+1, 1:nelz+1, 1);
...
%   llx = xPhys(elmNrs(:,z1+1:z2-1,x1:nelx));
%   mm = mean(sum(llx(:))) / numel(elmNrs(:,z1+1:z2-1,x1:nelx));
%   fprintf('v-beam / V = %f \n',mm);

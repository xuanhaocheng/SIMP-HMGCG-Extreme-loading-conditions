...
pasSS1 = setdiff(pasSS, intersection);

pasS = double(pasSS1);
vol = numel(pasS) / (nelx * nely * nelz);
...
volfrac0 = (volfrac * nEl - numel(pasS)*1.0 - numel(pasV) * 1e-6) / (nEl - numel(pasS)-numel(pasV) ) ;
x( act ) = volfrac0;
...
F = zeros(nDof,1);
volfrac_wall = 0.5;
volfrac_floor = 0.3;  %Obtain the force on each element node based on the density, quality, and overall force situation of the elements
...
lcDof15 = 3 * nodeNrs( 1 : nely + 1  , [zz+1, zz *2+1,nelz+1], [1, nelx+1])-2; 
...
F0(lcDof15) = nelx*nely*nelz*volfrac*g3*0.2/(3*2*numel(lcDof15));
F(:) = F0(:);
 F(fixed)=0.0;
dV( act, 1 ) = 1/nEl/volfrac; 
[ xPhys, xOld, ch, loop, U ] = deal( x, 1, 1, 0, zeros( nDof, 1 ) );       % old x, x change, it. counter, U
% ================================================= START OPTIMIZATION LOOP
nu=0.3; nswp=10; printLev=1; hx=1; hy=1; hz=1; cgtol=1e-5; cgmax=300;
...
filename = 'xphys_frame_data_8_vertical_solid_test.mat';
...
savefig(strcat('QNC_V1_staged_frame_test',num2str(loop),'.fig'));
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

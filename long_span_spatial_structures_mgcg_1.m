...
% Define the coordinates and radius of the spherical center
h = 18;                                                                    % x coordinate
k = 18;                                                                    % y coordinate
l = -8.4;                                                                  % z coordinate
r = 24;                                                                    % The radius of the ball
l1 = -12.6; 

pasVV = zeros(nely* nelx*nelz,1);
pasSS = zeros(nely* nelx*2,1);                                             % Initialize pass
pasS_index = zeros(nelx * nely, 3);                                        % Initialize pasS_index
pasV_index = zeros(nelx * nely*nelz, 3);                                   % Initialize pasS_index
index_count = 0;                                                           % Record the current number of processed indexes
index_count1 = 0;
% Traverse each fixed case of x and y
for i = 1:nelx
    for j = 1:nely
        % Calculate the distance from the center point of each unit to the sphere
        center_x = (i - 0.5) * 31.2 / nelx + 2.4;
        center_y = (j - 0.5) * 31.2 / nely + 2.4;
        center_z_jie =sqrt(r*r - (center_x - h).^2 -(center_y - k).^2) + l;
        center_z_jie1 = sqrt(r*r - (center_x - h).^2 -(center_y - k).^2) + l1;
        % Initialize the minimum distance and corresponding z-index
        minDistance = inf;
        minDistance1 = inf;
        minIndex = 0;
        minIndex1 = 0;
        ...
        % Traverse z
        for z = 1:nelz
            center_z = (z - 0.5) * 15.6 / nelz;
            if center_z < center_z_jie1
                newValue = (i-1)*nely*nelz + (z-1)*nely + j;
                pasVV((i-1)*nely*nelz + (z-1)*nely + j) = newValue;
                index_count1 = index_count1 + 1;
                pasV_index(index_count, :) = [i, j, z];
            end
            % calculate distance
            distance = abs(center_z - center_z_jie);
            distance1 = abs(center_z - center_z_jie1);
            % Update minimum distance and index
            if min(distance) < minDistance
                minDistance = min(distance);
                minIndex = z;
            end
            if min(distance1) < minDistance1
                minDistance1 = min(distance);
                minIndex1 = z;
            end
        end
        % Convert z index to global index
        global_index =  (i-1)*nely*nelz + (minIndex-1)*nely + j;
        global_index1 =  (i-1)*nely*nelz + (minIndex1-1)*nely + j;
        % Store in pass
        pasSS((i-1)*nely+j) = global_index;
        pasSS((i-1)*nely+j +nely* nelx ) = global_index;
         % Store in pass index
        index_count = index_count + 1;
        pasS_index(index_count, :) = [i, j, minIndex];
        pasS_index(index_count+1, :) = [i, j, minIndex1];
        index_count = index_count + 1;
    end
end
pasV_nonZero = pasVV(pasVV ~= 0);
pasV = pasV_nonZero;
commonIndices = ismember(pasV, pasSS);

% Delete elements corresponding to indexes in pasVV
pasV(commonIndices) = [];

% % Draw a scatter plot
% scatter3(pasV_index(:, 1), pasV_index(:, 2), pasV_index(:, 3), 'filled');
% xlabel('i');
% ylabel('j');
% zlabel('minIndex');
% title('Visualization of pasS\_index');
% 
% grid on;

% scatter3(pasS_index(:, 1), pasS_index(:, 2), pasS_index(:, 3), 'filled');
% xlabel('i');
% ylabel('j');
% zlabel('minIndex');
% title('Visualization of pasS\_index');
...
Assembly_F0_1(F0,pasS, cVec,g3, nelx, nely, nelz,direction3);              %Assembly_F0_ Before loop
% Assembly_F0_1(F0,pasS, cVec,g2, nelx, nely, nelz,direction2);
% Assembly_F0_1(F0,pasS, cVec,g1, nelx, nely, nelz,direction1);
volfrac=(numel(act)*volfrac0+numel(pasS)*0.001+numel(pasV)*0.0)/nEl;
...
filename = 'xphys_long_span_data.mat';
...
savefig(strcat('QNC_V1_long_span',num2str(loop),'.fig'));
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
